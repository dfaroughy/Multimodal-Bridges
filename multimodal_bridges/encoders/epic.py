...
class EPiCNetwork(nn.Module):
    def __init__(
        ...
        self.continuous_head = torch.nn.Sequential(
                                weight_norm(torch.nn.Linear(dim_hidden_local//2, dim_hidden_local//2)),
                                torch.nn.GELU(),
                                weight_norm(torch.nn.Linear(dim_hidden_local//2, dim_output_continuous)),
                            )

        self.discrete_head = torch.nn.Sequential(
                                weight_norm(torch.nn.Linear(dim_hidden_local//2, dim_hidden_local//2)),
                                torch.nn.GELU(),
                                weight_norm(torch.nn.Linear(dim_hidden_local//2, dim_output_discrete)),
                            )
    ...
    def forward(self, x_local, context=None, mask=None):
        ...
        h_continuous = self.continuous_head(h1) * mask
        h_discrete = self.discrete_head(h2) * mask
