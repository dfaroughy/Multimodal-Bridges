...
class EPiCEncoderFused(nn.Module):
    def __init__(
        ...
        self.epic_body = EPiCProjection(
            ...
            dim_loc=dim_input,
            dim_glob=dim_context,
            dim_hid_loc=2 * dim_hid_loc,
            dim_hid_glob=2 * dim_hid_glob,
            pooling_fn=self._meansum_pool,
            dropout=dropout,
        )
        ...
        self.continuous_output = nn.Sequential(
            wn(nn.Linear(dim_time + dim_hid_loc + dim_hid_glob, dim_output_continuous)),
        )

        self.discrete_output = nn.Sequential(
            wn(nn.Linear(dim_time + dim_hid_loc + dim_hid_glob, dim_output_discrete)),
        )
    ...
    def forward(self, time_local, x_local, context=None, mask=None):
        ...
        z_local, z_global = self.epic_body(time_local, x_local, context, mask)
        ...
        for i in range(self.num_blocks):
            x_local, x_global = self.epic_continuous_head[i](x_local, x_global, context, mask)
            k_local, k_global = self.epic_discrete_head[i](k_local, k_global, context, mask)
            x_local += x_local_skip
            x_global += x_global_skip
            k_local += k_local_skip
            k_global += k_global_skip

        # ...outputs

        x_global = self._broadcast_global(x_global, local=x_local)
        k_global = self._broadcast_global(k_global, local=k_local)

        h_continuous = torch.cat([time_local, x_local, x_global], dim=-1)
        h_continuous = self.continuous_output(h_continuous)

        h_discrete = torch.cat([time_local, k_local, k_global], dim=-1)
        h_discrete = self.discrete_output(h_discrete)
